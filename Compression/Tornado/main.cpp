...
check_for_errors:
    ...
    if (r.method.match_finder%10 >= 5  &&  (r.method.auxhash_size==0 || r.method.auxhash_row_width==0)) {
        char ah[100];  showMem64(r.method.auxhash_size,ah);
        fprintf (stderr, "ERROR: empty auxhash (-ah%s -al%d) while MINLEN=%d (-x%d)\n", ah, r.method.auxhash_row_width, r.method.match_finder%10, r.method.match_finder);
        exit(FREEARC_EXIT_ERROR);
    }
